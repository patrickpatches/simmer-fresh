...
const CHICKEN_ADOBO: Recipe = {
  ...
  ingredients: [
    ...
    { id: 'i8', name: 'Steamed white rice, to serve', amount: 300, unit: 'g (dry)', scales: 'linear', prep: 'Start cooking at the beginning of s2 — the 25-min braise gives exactly enough time' },
  ],
  steps: [
    ...
    { id: 's4', title: 'Serve over rice', content: 'Start your rice at the beginning of s2 — it will be ready when the sauce finishes reducing. Serve immediately over steamed white rice. The sauce is the point — make sure every plate gets a generous spoonful.' },
  ],
...
const ROAST_CHICKEN: Recipe = {
  ...
  base_servings: 4,
  time_min: 1590, // 90 min active cook + overnight dry brine (24h min commitment)
  difficulty: 'Easy',
...
const SOURDOUGH_LOAF: Recipe = {
  ...
  steps: [
    ...
    { id: 's5', title: 'Bake in a covered pot at maximum heat', content: 'Preheat oven to 250°C (as hot as it goes) with a Dutch oven inside for 1 hour. Score the cold loaf with a sharp razor or lame. Put it straight from the fridge into the scorching Dutch oven. Cover and bake 20 minutes. Remove lid, bake 20–25 more minutes until deep mahogany. Internal temperature should hit 96°C.', timer_seconds: 2400, why_note: 'The covered Dutch oven traps steam released by the cold dough in the first 20 minutes. This steam keeps the crust supple enough to expand fully (oven spring). At 20 minutes, the crust has set — removing the lid now allows the Maillard reaction on the crust surface and the deep caramelisation that makes sourdough crust flavour.' },
    { id: 's6', title: 'Rest — do not cut yet', content: 'Transfer the loaf to a wire rack. Leave uncovered for at least 1 hour before cutting. Do not slice early, no matter how good it smells.', timer_seconds: 3600, why_note: 'The interior continues cooking via carry-over heat for 20–30 minutes after leaving the oven. More importantly, the crumb structure is still setting — it is essentially liquid gel in the centre when first removed. Cut at 20 minutes and the inside is gummy and underbaked-looking even though the crust temperature was correct. Cut at 1 hour and the crumb is open, set, and properly structured.' },
  ],
  categories: { cuisines: ['australian'], types: ['baking'] },
  whole_food_verified: true,
};
...
const RAMEN: Recipe = {
  ...
  ingredients: [
    ...
    { id: 'i7', name: 'Chashu pork belly (or roasted pork)', amount: 100, unit: 'g', scales: 'linear', prep: 'Make chashu ahead (minimum 2–3h, ideally the day before) — this recipe has no chashu-making step. Shortcut: buy char siu from a Chinese BBQ shop, slice thinly.',
      substitutions: [
...
const BEEF_RENDANG: Recipe = {
  ...
  steps: [
    { id: 's1', title: 'Make the rempah (spice paste)', content: 'Before anything else: pour boiling water over the dried chillies and soak 20 minutes — they need to be fully soft before blending. Once soaked, drain and add to the blender with lemongrass, galangal, fresh chillies, shallots, garlic, candlenuts, turmeric, and 2 kaffir lime leaves. Add a splash of water and blend until completely smooth — 3–4 minutes in a high-powered blender or longer with a stick blender.', timer_seconds: 240, why_note: 'Rempah is the foundation. Every flavour in the rendang builds from this paste. The dried chilli soak is non-negotiable — unsoaked dried chillies will not blend smooth and leave tough fibrous pieces. Inadequate blending also leaves fibrous lemongrass chunks that never fully cook out. If you don\'t have a powerful blender, slice everything finely and then blend — it gives the blades something to grip.' },
    ...
    { id: 's4b', title: 'Make kerisik during the braise', content: 'Any time during the first 90 minutes of the braise: toast desiccated coconut in a dry pan over medium heat, stirring constantly, 8–10 minutes until deep golden-brown. Watch it — it goes from golden to burnt in 30 seconds. Tip into a mortar and pound vigorously for 2–3 minutes until it resembles a paste and smells intensely nutty. The friction releases the coconut oil — you will see it glistening. Set aside.', timer_seconds: 600, why_note: 'Kerisik is toasted, pounded coconut that has released its own oil through friction. It adds the distinctive nutty, slightly textured quality that distinguishes proper rendang from a coconut milk curry. The pounding is the step — unpounded toasted coconut stays granular and dry and does not integrate into the sauce the same way. This step takes 10–15 minutes and is best done while the braise looks after itself.' },
...
const CURRY_LAKSA: Recipe = {
  ...
  steps: [
    ...
    { id: 's3b', title: 'Pan-fry the tofu', content: 'In a separate non-stick pan, heat 1 tbsp oil over high heat. Add tofu cubes in a single layer — do not crowd the pan. Fry 2–3 minutes per side until all faces are golden and the tofu has a firm outer crust. Set aside. The tofu goes into the broth later.', timer_seconds: 300, why_note: 'Tofu straight from the packet is wet and bland — it absorbs rather than holds flavour. Frying first achieves two things: it drives off surface moisture so the tofu holds its shape in the broth (unfried tofu breaks apart), and the golden crust adds flavour and a slightly chewy texture that contrasts with the soft interior.' },
...
const BUTTER_CHICKEN: Recipe = {
  ...
  base_servings: 4,
  time_min: 330, // 4h minimum marinade + 60–75 min active cook. Plan the day before.
  difficulty: 'Intermediate',
...
const SAAG_PANEER: Recipe = {
  ...
  source: {
    chef: 'Hone Kitchen',
    notes: 'Technique adapted from multiple sources. Uses fresh spinach — not frozen.',
  },
  emoji: '🌿',
...
const BARRAMUNDI: Recipe = {
  ...
  base_servings: 4,
  time_min: 50, // includes 30 min hands-off skin-drying step in the fridge (mandatory for crispy skin)
  difficulty: 'Easy',
...
const PAVLOVA: Recipe = {
  ...
  base_servings: 1,
  yield_unit: 'pavlova',
  time_min: 210, // 30 min egg-white rest + 90 min baking + 60 min oven cooling (minimum). Often 4h total.
  difficulty: 'Intermediate',
...
const FLOUR_TORTILLAS: Recipe = {
  ...
  steps: [
    ...
    },
    {
      ...
      why_note: 'High-protein flour needs relaxation time or 